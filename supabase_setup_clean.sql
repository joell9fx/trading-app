-- Trading Academy Community Platform Database Setup (Clean Version)
-- This script will drop existing objects and recreate them
...
DROP INDEX IF EXISTS idx_bookings_mentor_id;
...
DROP POLICY IF EXISTS "Users can view all signals" ON signals;
DROP POLICY IF EXISTS "Only admins can create signals" ON signals;
DROP POLICY IF EXISTS "Only admins can update signals" ON signals;
DROP POLICY IF EXISTS "Only admins can delete signals" ON signals;
...
DROP POLICY IF EXISTS "Users can view all mentors" ON mentors;
DROP POLICY IF EXISTS "Users can update own mentor profile" ON mentors;
DROP POLICY IF EXISTS "Users can create own mentor profile" ON mentors;
...
-- Insert sample courses (no user dependencies)
